...
mod parser_test {
    ...
    fn compile(src: &str) -> compiler::modules::parser::Chunk {
    ...
    fn test_cases() {
        ...
        for (src, expected_constants, expected_names, expected_instructions) in cases {
            ...
            let got_constants: Vec<String> = chunk.constants.iter().map(|v| match v {
                Value::Str(s) => s.clone(),
            ...
            let got_instructions: Vec<(String, u16)> = chunk.instructions.iter()
                .map(|i| (format!("{:?}", i.opcode), i.operand))
                .collect();

            assert_eq!(got_constants, expected_constants, "constants mismatch on: {:?}", src);
            assert_eq!(chunk.names, expected_names, "names mismatch on: {:?}", src);
